...
public abstract class Load {
	...
	private static IImage loadAndScaleImage(String url, float scale, IFileFactory ff) {
		...
		IImage img = ImageFactory.instance.loadImage(file);
		IImage scaled = ImageFactory.instance.createScaled(img, scale, scale);
		return scaled;
	}
	...
	public static IImage triangleImage(TsColor color, int blockSize) {
		...
		IImage image = ImageFactory.instance.createEmpty(size, size);
	...
	private static void spriteSpawnHole(int blockSize) {
		...
		IImage image = ImageFactory.instance.createEmpty(blockSize, blockSize);
	...
	private static void spriteProjectile(int blockSize) {
		...
		IImage image = ImageFactory.instance.createEmpty(width, height);
		IGraphics g = new ScaledGraphics(image.getGraphics(), blockSize / 50.0f);
		g.setColor(255, 80, 0);
		g.fillRect(0, 0, 2, 10);
		g.dispose();
	...
	private static void spriteHQ(int blockSize) {
		...
		IImage image = ImageFactory.instance.createEmpty(blockSize * 2, blockSize * 2);
		IGraphics g = new ScaledGraphics(image.getGraphics(), blockSize / 50.0f);
		g.setAntiAlias(true);
		g.setColor(TsColor.darkGray);
